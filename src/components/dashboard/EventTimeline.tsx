...
function getEventIcon(event: TimelineEvent) {
  ...
  if (model === "gemini-2.5-flash-native-audio")
  ...
  if (text.includes("resolv") || text.includes("complet"))
    return <CheckCircle size={12} className="text-emerald-400" />;
  if (text.includes("call") || text.includes("calling"))
    return <Mic size={12} className="text-orange-400" />;
  if (text.includes("search") || text.includes("found"))
    return <Search size={12} className="text-blue-400" />;
  if (text.includes("analyz") || text.includes("recommend"))
