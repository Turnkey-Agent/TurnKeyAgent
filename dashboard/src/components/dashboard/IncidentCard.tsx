...
import { AlertTriangle, Wrench, Zap, Wind, Package, Building, ChevronDown } from "lucide-react";
...
export function IncidentCard({ incident, unitNumber, propertyName, onClick }: IncidentCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className={cn(
        "rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5 transition-colors",
        onClick && "cursor-pointer hover:border-blue-500/40 hover:bg-[var(--surface-2)]"
      )}
    >
      {/* Header row */}
